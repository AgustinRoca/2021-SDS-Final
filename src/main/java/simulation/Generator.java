...
public class Generator {
    ...
    public static List<List<Cell>> generateMatrix(int rows, int columns, double roomTemperature, double cellSize, double treeRatio){
            ...
            for (int j = 0; j < columns; j++) {
                if(Math.random() < treeRatio){
                    matrix.get(i).add(new Cell(
                            new Tree(new Position(j*cellSize, (rows - 1 - i)*cellSize), roomTemperature, roomTemperature)));
                } else {
        ...
        Tree burningTree = new Tree(
                new Position((columns/2)*cellSize, (rows - 1 - (rows/2))*cellSize), Tree.getStartingTemperature(), roomTemperature);
